...
const FittsTaskStandard = () => {
  ...
  useEffect(() => {
    if (experimentComplete) {
      const completeExperiment = async () => {
        ...
        try {
          await FlowHelper.setVisited("FittsTaskStandard"); // Keep Standard-specific
  ...
  const handleContainerClick = (e) => {
    ...
    if (!trialData) {
      setTrialData({
        UserID: userId,
        MausTyp: "Standard", // Keep Standard-specific
    ...
    if (onTarget) {
      ...
      // Check if we've completed all clicks for this difficulty
      if (updatedClickCount >= NUM_CLICKS_PER_TRIAL) {
        setShowLevelComplete(true);
      }
    } else {
      // Error case - update trial data to mark as error but don't move to next target
      setTrialData(prevData => {
        if (prevData) {
          // We already have error data for this trial, just update the click position and time
          return {
            ...prevData,
            MT: movementTime,
            ClickX: clickX,
            ClickY: clickY,
            Error: 1 // Always mark as error
          };
        } else {
          // First error for this trial
          return {
            UserID: userId,
            MausTyp: "Standard", // Keep Standard-specific
            TargetID: activeTargetId,
  ...
  return (
    <div>
      ...
      {showMouseInfo && (
        <MouseInfoPopup mouseType="Standard" onClose={handleMouseInfoClose} />
      )}
      <h2>Klick-Task: Standard-Mauseinstellung</h2>
...
export default FittsTaskStandard;
...
const FittsTaskML = () => {
  ...
  useEffect(() => {
    if (experimentComplete) {
      const completeExperiment = async () => {
        ...
        try {
          await FlowHelper.setVisited("FittsTaskML");
  ...
  const sendResults = async () => {
    try {
      ...
      const formattedTrials = trials.slice(0, NUM_DIFFICULTIES * NUM_CLICKS_PER_TRIAL).map(trial => ({
        ...
        ClickX: trial.ClickX,
        ClickY: trial.ClickY,
        TargetX: trial.TargetX, // Include target X in the formatted data
        Width: trial.Width,
  ...
  return (
    <div>
      ...
      {showMouseInfo && (
        <MouseInfoPopup mouseType="ML" onClose={handleMouseInfoClose} />
      )}
      <h2>Klick-Task: Maus mit ML-Optimierung</h2>
